<%-- 
    ...
    Created on : May 13, 2017, 6:11:38 AM
    ...
    <body>
        ...
        <sql:query dataSource = "${snapshot}" var = "info">
            <c:forEach var = "info_1" items = "${info1.rows}">
                SELECT customer.firstName, customer.lastName, booking.reserved_date, booking.bookingId, booking.bookingStatus, service.serviceType FROM booking NATURAL JOIN service NATURAL JOIN customer WHERE booking.spId='<c:out value = "${info_1.idNum}"/>' AND booking.bookingStatus='accepted' OR booking.bookingStatus='ongoing' ORDER BY booking.bookingId DESC;
            </c:forEach>
        </sql:query>
        ...
        <div id="requests">
            ...
            <form action="CancelServlet" method="POST">
                <c:forEach var = "requests" items = "${info.rows}">
                    <p><input type="hidden" name="bookingId" value=<c:out value = "${requests.bookingId}"/> readonly><c:out value = "${requests.firstName}"/> <c:out value="${requests.lastName}"/>: <c:out value="${requests.reserved_date}"/> <c:out value="${requests.serviceType}"/> <b>Current Status:</b> <c:out value="${requests.bookingStatus}"/> <input type="submit" name="cancel" value="Cancel"></p>
                </c:forEach>
            </form>
        </div>
