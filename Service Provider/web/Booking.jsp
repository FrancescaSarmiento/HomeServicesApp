<%-- 
    Document   : Booking
    Created on : May 13, 2017, 2:14:04 AM
    ...
    <body>
        ...
        <sql:query dataSource = "${snapshot}" var = "info">
            <c:forEach var = "info_1" items = "${info1.rows}">
                SELECT customer.firstName, customer.lastName, booking.reserved_date, service.serviceType, booking.bookingId FROM booking NATURAL JOIN service NATURAL JOIN customer WHERE booking.spId='<c:out value = "${info_1.idNum}"/>' AND booking.bookingStatus='pending' ORDER BY booking.bookingId DESC;
            </c:forEach>
        </sql:query>
        ...
        <div id="requests">
            ...
            <form action="AcceptServlet" method="POST">
                <c:forEach var = "requests" items = "${info.rows}">
                    <p><input type="hidden" name="bookingId" value=<c:out value = "${requests.bookingId}"/> size="1" readonly> <c:out value = "${requests.firstName}"/> <c:out value="${requests.lastName}"/>: <c:out value="${requests.reserved_date}"/> <c:out value="${requests.serviceType}"/> <input type="submit" value="Accept"></p>
                </c:forEach>
            </form>
        </div>
