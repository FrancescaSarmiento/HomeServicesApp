        ...
        <div id="requests">
            ...
            <form action="FinishServlet" method="POST">
                <c:forEach var = "requests" items = "${info.rows}">
                    <p><input type="hidden" name="bookingId" value=<c:out value = "${requests.bookingId}"/> readonly><c:out value = "${requests.firstName}"/> <c:out value="${requests.lastName}"/>: <c:out value="${requests.reserved_date}"/> <c:out value="${requests.serviceType}"/> <input type="submit" name="timeFinished" value="Finish"></p>
                </c:forEach>
            </form>
        </div>
